...
PartDataBase::PartDataBase(QString file_name)
    : m_file_name(file_name), m_category_nodes("") {
...
void PartDataBase::load_categories_recursive(
    PartCategoryTreeNode &categories_recursion, QString root,
...
QStringList PartDataBase::get_categories_by_json(QString categorie_root) {
  ...
  auto root_cat_path =
      PartCategoryTreeNode::split_category_path(categorie_root);
...
PartDataBase::get_category_node_ref(QString categorie_path) {
#if 1
  qDebug() << "queried:" << categorie_path;
  return m_category_nodes.get_category(categorie_path);
#endif
...
PartCategoryTreeNode::get_categorie_by_path_recursion(QStringList path,
  ...
  if (is_leaf) {
    ...
    return get_child(path_element)
        .get_categorie_by_path_recursion(path, depth + 1);
  }
}

void PartCategoryTreeNode::get_partids_of_subcategories_recursive(
    QVector<int> &part_ids) const {
